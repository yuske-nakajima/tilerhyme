...
const CHAR_SETS = {
  // 全角英数字記号
  zenkaku:
    '１２３４５６８９０ＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚ',
  // ひらがな
  hiragana:
...
async function createBitmapFont(inputTTF, outputJSON) {
  try {
    // フォントデータの読み込み
    const fontData = await fs.readFile(inputTTF)

    // キャンバスの設定
    const size = 8 // 8x8ビットマップ
    ...
    registerFont(inputTTF, { family: fontName })

    // ビットマップデータ保存用オブジェクト
    const bitmaps = {}

    // 全文字セットの処理
    for (const [setName, chars] of Object.entries(CHAR_SETS)) {
      console.log(`Processing ${setName}... (${chars.length} characters)`)
    ...
    // メタデータの追加
    const metadata = {
      fontName: path.basename(inputTTF),
      size: size,
      timestamp: new Date().toISOString(),
      charCount: Object.keys(bitmaps).length,
      charSets: Object.keys(CHAR_SETS),
      charSetSizes: Object.fromEntries(Object.entries(CHAR_SETS).map(([key, value]) => [key, value.length])),
    }
    ...
    // JSONファイルとして保存
    await fs.writeFile(outputJSON, JSON.stringify(jsonData, null, 2), 'utf8')

    console.log('\nSummary:')
    console.log(`Font: ${metadata.fontName}`)
    console.log(`Total characters: ${metadata.charCount}`)
    for (const [set, count] of Object.entries(metadata.charSetSizes)) {
      console.log(`- ${set}: ${count} characters`)
    }
...
if (process.argv.length !== 4) {
  console.log('Usage: node script.js input.ttf output.json')
  process.exit(1)
}

const inputTTF = process.argv[2]
const outputJSON = process.argv[3]

createBitmapFont(inputTTF, outputJSON)
